...
try {
    ...
    if ($conn->connect_error) {
        ...
        $sql = "SELECT COUNT(DISTINCT b.id) as trip_count 
                FROM buses b 
                WHERE b.operator_name = ? AND DATE(b.journey_date) = CURDATE() AND b.status = 'Upcoming'";
        ...
        $sql = "SELECT COALESCE(SUM(bk.fare), 0) as total_fare 
                FROM bookings bk
                JOIN buses b ON bk.bus_id = b.id
                WHERE b.operator_name = ? AND bk.status IN ('Upcoming', 'Completed')";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("s", $company_name);
        $stmt->execute();
        $result = $stmt->get_result();
        ...
        if ($is_bus_filter) {
            ...
            $sql = "SELECT b.bus_number, COALESCE(SUM(bk.fare), 0) as revenue 
                    FROM buses b 
                    LEFT JOIN bookings bk ON b.id = bk.bus_id AND bk.status IN ('Upcoming', 'Completed')
                    WHERE b.operator_name = ? 
            ...
            if ($filter_type === 'date') {
                $select_fields = "DATE(bk.date) as label";
                $group_by = "GROUP BY DATE(bk.date)";
                $order_by = "ORDER BY label ASC";
                $x_title = 'Date';
                $dataset_label = 'Daily Revenue (Tk)';
            } elseif ($filter_type === 'month') {
                $select_fields = "CONCAT(YEAR(bk.date), '-', LPAD(MONTH(bk.date), 2, '0')) as label";
                $group_by = "GROUP BY YEAR(bk.date), MONTH(bk.date)";
                $order_by = "ORDER BY YEAR(bk.date) ASC, MONTH(bk.date) ASC";
                $x_title = 'Month';
                $dataset_label = 'Monthly Revenue (Tk)';
            } elseif ($filter_type === 'year') {
                $select_fields = "YEAR(bk.date) as label";
                $group_by = "GROUP BY YEAR(bk.date)";
                $order_by = "ORDER BY label ASC";
                $x_title = 'Year';
                $dataset_label = 'Yearly Revenue (Tk)';
            }

            $sql = "SELECT $select_fields, COALESCE(SUM(bk.fare), 0) as revenue 
                    FROM bookings bk
                    JOIN buses b ON bk.bus_id = b.id
                    WHERE b.operator_name = ? AND bk.status IN ('Upcoming', 'Completed')
                    $group_by
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add_trip'])) {
            ...
            if (!empty($errors)) {
                ...
                $sql = "INSERT INTO buses (operator_name, bus_number, bus_type, starting_point, destination, starting_time, arrival_time, fare, seats_available, journey_date, status) 
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'Upcoming')";
                $stmt = $conn->prepare($sql);
                ...
                <div class="revenue-table">
                    ...
                    <?php else: ?>
                        <table>
                            <thead>
                                <tr>
                                    <th><?php echo htmlspecialchars($x_title); ?></th>
                                    <th>Revenue (Tk)</th>
                                </tr>
                            </thead>
                            <tbody>
        ...
        <div class="content-tabs" id="trips">
            <div class="add-trip-form">
                ...
                <form id="searchTripForm" method="POST" action="">
                    ...
                    <div class="form-group">
                        <label><span style="color: red; font-size: 20px;">*</span>Users will get an update from their Dashboard.</label>
