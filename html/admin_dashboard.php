...
$existing_columns = [];
while ($row = mysqli_fetch_assoc($column_check)) {
...
while ($row = mysqli_fetch_assoc($column_check)) {
    $existing_feedbackadmin_columns[] = $row['Field'];
}
$missing_feedbackadmin_columns = array_diff($required_feedbackadmin_columns, $existing_feedbackadmin_columns);
if (!empty($missing_feedbackadmin_columns)) {
...
while ($row = mysqli_fetch_assoc($column_check)) {
    $existing_feedback_columns[] = $row['Field'];
}
$missing_feedback_columns = array_diff($required_feedback_columns, $existing_feedback_columns);
if (!empty($missing_feedback_columns)) {
...
while ($row = mysqli_fetch_assoc($column_check)) {
    $existing_complaints_columns[] = $row['Field'];
}
$missing_complaints_columns = array_diff($required_complaints_columns, $existing_complaints_columns);
if (!empty($missing_complaints_columns)) {
...
while ($row = mysqli_fetch_assoc($column_check)) {
    $existing_company_columns[] = $row['Field'];
}
$missing_company_columns = array_diff($required_company_columns, $existing_company_columns);
if (!empty($missing_company_columns)) {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add_company'])) {
    ...
    if (empty($company_name)) {
        $company_errors[] = "Company name is required.";
    } elseif (strlen($company_name) > 255) {
        $company_errors[] = "Company name must be 255 characters or less.";
    }

    if (empty($phone)) {
        $company_errors[] = "Phone number is required.";
    } elseif (!preg_match('/^\d{11}$/', $phone)) {
        $company_errors[] = "Phone number must be exactly 11 digits.";
    }

    if (empty($password)) {
        $company_errors[] = "Password is required.";
    } elseif (strlen($password) < 6) {
        $company_errors[] = "Password must be at least 6 characters.";
    }
    ...
    } elseif (mysqli_num_rows($check_company) > 0) {
    ...
    if (empty($company_errors)) {
        ...
        if (!$stmt) {
            $company_errors[] = "Prepare failed: " . mysqli_error($conn);
        } else {
            $stmt->bind_param("sss", $company_name, $phone, $hashed_password);
            if ($stmt->execute()) {
                $company_success = "Bus company added successfully!";
            } else {
                $company_errors[] = "Error adding bus company: " . mysqli_error($conn);
            }
            $stmt->close();
        }
    }
    ...
    if (empty($report_errors)) {
        ...
        if ($report_type === 'Revenue Report' || $report_type === 'Sales Report') {
            ...
            if (mysqli_num_rows($column_check) == 0) {
                $report_errors[] = "Error: The 'status' column is missing in the bookings table.";
            }
        }
        ...
        if (empty($report_errors)) {
            if ($report_type === 'User Feedback Report') {
                ...
                if ($date_range === 'Last 7 Days') {
                    ...
                    $date_filter = "WHERE created_at >= CURDATE() - INTERVAL 30 DAY";
                ...
                if ($date_range === 'Last 7 Days') {
                    ...
                    $date_filter = "WHERE b.date >= CURDATE() - INTERVAL 30 DAY AND b.status IN ('Completed', 'Upcoming')";
                } elseif ($date_range === 'Custom') {
                    $date_filter = "WHERE b.date BETWEEN ? AND ? AND b.status IN ('Completed', 'Upcoming')";
                } else {
                    $date_filter = "WHERE b.status IN ('Completed', 'Upcoming')";
                }
            }
        }
    }
        ...
        try {
            if ($report_type === 'Revenue Report') {
                ...
                if ($date_range === 'Custom') {
                    ...
                    $stmt->bind_param("ss", $custom_start, $custom_end);
                    $stmt->execute();
                    ...
                    if (!$report_data) {
                        throw new Exception("Error generating revenue report: " . mysqli_error($conn));
                    }
                }
                ...
                if ($date_range === 'Custom') {
                    ...
                    $report_data = mysqli_query($conn, $query);
                    if (!$report_data) {
                        throw new Exception("Error generating sales report: " . mysqli_error($conn));
                    }
                }
                ...
                if ($date_range === 'Custom') {
                    ...
                    $report_data = mysqli_query($conn, $query);
                    if (!$report_data) {
                        throw new Exception("Error generating feedback report: " . mysqli_error($conn));
                    }
                }
            }
        } catch (Exception $e) {
            $report_errors[] = $e->getMessage();
        }
    }
}
...
$revenue_query = "SELECT b.route AS route_name, 
                         ...
                         b.fare AS avg_price 
                  FROM bookings b
                  WHERE b.status IN ('Completed', 'Upcoming')
                  GROUP BY b.route, b.fare 
                  HAVING tickets_sold > 0";
try {
    $revenue_result = mysqli_query($conn, $revenue_query);
    if (!$revenue_result) {
        throw new Exception("Error fetching revenue data: " . mysqli_error($conn));
    }
} catch (Exception $e) {
    die($e->getMessage());
}
...
$feedback_query = "
    ...
    UNION
    (SELECT f.id, 'Feedback' AS item_type, u.username AS user_name, f.booking_id, b.route, 
            f.rating AS feedback_type, f.comments AS feedback_text, f.created_at, NULL AS status, 
    ...
    ORDER BY FIELD(item_type, 'Complaint', 'Feedback'), 
             FIELD(status, 'Pending', 'Resolved', 'Dismissed', NULL), 
             created_at DESC";
                ...
                <?php while ($item = mysqli_fetch_assoc($feedback_result)) { ?>
                    <div class="feedback-item">
                        <h4>
                            ...
                            <?php if ($item['item_type'] === 'Complaint' || ($item['item_type'] === 'Feedback' && $item['booking_id'] !== null)) { ?>
                                (Booking ID: <?php echo htmlspecialchars($item['booking_id'] ?? 'N/A'); ?>, 
                                Route: <?php echo htmlspecialchars($item['route'] ?? 'N/A'); ?>
                                <?php if ($item['item_type'] === 'Complaint') { ?>
                                    , Status: <?php echo htmlspecialchars($item['status'] ?? 'N/A'); ?>
                                <?php } ?>)
                            <?php } ?>
                        </h4>
                        <p><strong><?php echo $item['item_type'] === 'Feedback' && $item['booking_id'] !== null ? 'Rating' : 'Type'; ?>:</strong> 
                           <?php echo htmlspecialchars($item['feedback_type']); ?>
                           <?php if ($item['item_type'] === 'Feedback' && $item['booking_id'] !== null) { ?>
                               (<?php echo htmlspecialchars($item['feedback_type']); ?>/5)
                           <?php } ?>
                        </p>
                        <p><strong>Details:</strong> <?php echo htmlspecialchars($item['feedback_text']); ?></p>
...
// Verify feedbackadmin table schema
$required_feedback_columns = ['id', 'user_name', 'feedback_text', 'feedback_type', 'created_at'];
$column_check = mysqli_query($conn, "SHOW COLUMNS FROM feedbackadmin");
$existing_feedback_columns = [];
while ($row = mysqli_fetch_assoc($column_check)) {
    $existing_feedback_columns[] = $row['Field'];
...
$missing_feedback_columns = array_diff($required_feedback_columns, $existing_feedback_columns);
if (!empty($missing_feedback_columns)) {
    die("Error: Missing columns in feedbackadmin table: " . implode(", ", $missing_feedback_columns) . ". Please update the database schema.");
}
